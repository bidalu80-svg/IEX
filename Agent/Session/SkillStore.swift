...
final class SkillStore: ObservableObject {
    ...
    private init() {
        ...
        createTables()
        loadSkills()
        removeLegacyBundledSkillCreator()
    ...
    // MARK: - Legacy Bundled Skill Cleanup

    /// Earlier builds silently installed `skill-creator`. Remove only that
    /// app-bundled instance once; skills created or imported by a user stay
    /// untouched and the normal create/import flows remain available.
    private func removeLegacyBundledSkillCreator() {
        let bundledId = "skill-creator"
        guard let legacy = skills.first(where: {
            $0.id == bundledId && $0.importSource == .bundled
        }) else { return }

        skills.removeAll { $0.id == bundledId }
        dbDeleteSkill(id: bundledId)
        try? fm.removeItem(at: skillsDir.appendingPathComponent(bundledId))
        try? fm.removeItem(at: rootfsSkillsDir.appendingPathComponent(bundledId))
        AppLogger(category: "SkillStore").info("Removed legacy bundled skill '\(legacy.name)'")
        Task {
            await ChatStore.shared.markDirty(
                recordType: "Skill", recordId: bundledId, operation: "delete"
            )
        }
    }
