...
extension AIChatViewModel {
    ...
    func loadSession() async {
        ...
        guard !rawMessages.isEmpty else {
            ...
            // Still mount ze even for empty sessions so /var/ze works in terminal
            logger.info("🔍MOUNT loadSession empty-session fallback, still mounting for \(sessionId)")
            mountZe(for: sessionId)
            return
        }
        ...
        // Phase 4: Migrate offloads + mount ze
        ...
        logger.info("🔍MOUNT loadSession Phase4 calling mountZe for \(sessionId) (msgs=\(rawMessages.count))")
        mountZe(for: sessionId)
