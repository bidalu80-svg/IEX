...
/// Shared percent-decoding helpers for resolving `ze://` URLs to on-disk
/// subpaths.
///
/// A correctly-formed `ze_url` percent-encodes the filename exactly once
/// (see `linuxPathToZeURL`), so `URL(string:).path` decodes it back to the
...
enum ZeURLPathDecoding {
    /// Candidate subpaths for a `ze://` URL, in priority order.
