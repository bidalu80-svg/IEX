...
// MARK: - Request-level image budget + Ze paths + tool-output offload
...
extension AIChatViewModel {
    ...
    /// directory and return the iSH-visible `/var/ze/attachments/spillover/...`
    ...
    private static func ensureSpillover(_ data: Data, mimeType: String, sessionId: String) -> String? {
        ...
        let attachmentsRoot = zePersistentBase
        ...
        return "/var/ze/attachments/spillover/\(sha).\(ext)"
    }
    ...
    nonisolated static func zeWorkspacePersistentDir(for sid: String) -> URL {
        zePersistentBase.appendingPathComponent(sid, isDirectory: true)
            .appendingPathComponent("workspace", isDirectory: true)
    }

    /// Persistent storage directory for a specific session's browser snapshots.
    nonisolated static func zeBrowserPersistentDir(for sid: String) -> URL {
        zePersistentBase.appendingPathComponent(sid, isDirectory: true)
            .appendingPathComponent("browser", isDirectory: true)
    }

    nonisolated static var zeAppGroupRoot: URL {
        SharedContainerStore.persistentContainerURL
            .appendingPathComponent("ZeFileProvider", isDirectory: true)
    }
    ...
    nonisolated static var zeConfigRoot: URL {
        let url = SharedContainerStore.persistentContainerURL
            .appendingPathComponent("ZeConfig", isDirectory: true)
    ...
    nonisolated static var zeMemoryPersistentDir: URL {
        zeAppGroupRoot.appendingPathComponent("memory", isDirectory: true)
    }

    /// Persistent storage directory for skills (shared across all sessions).
    /// Stored in the App Group container so the FileProvider extension can access it.
    nonisolated static var zeSkillsPersistentDir: URL {
        zeAppGroupRoot.appendingPathComponent("skills", isDirectory: true)
    }

    /// Persistent storage directory for shared files (shared across all sessions).
    /// Stored in the App Group container so the FileProvider extension can access it.
    nonisolated static var zeSharedPersistentDir: URL {
        zeAppGroupRoot.appendingPathComponent("shared", isDirectory: true)
    }

    /// Persistent storage directory for MCP server configs (servers.json,
    /// daemon log). Deliberately under ZeConfig, NOT zeAppGroupRoot:
    ...
    nonisolated static var zeMcpServersPersistentDir: URL {
        zeConfigRoot.appendingPathComponent("mcp-servers", isDirectory: true)
    }
    ...
    nonisolated static func resolveZeURL(_ url: URL) -> URL? {
        ...
        if let sid = activeSessionId {
            for subPath in subPaths {
                let candidate = zePersistentBase
        ...
        let globalDirs: [(String, URL)] = [
            ("skills", zeSkillsPersistentDir),
            ("memory", zeMemoryPersistentDir),
            ("shared", zeSharedPersistentDir),
        ]
        ...
        // Scan all sessions
        if let sessions = try? fm.contentsOfDirectory(atPath: zePersistentBase.path) {
            for sid in sessions {
                for subPath in subPaths {
                    let candidate = zePersistentBase
                        .appendingPathComponent(sid, isDirectory: true)
    ...
    func offloadToolOutput(_ output: String, toolName: String, toolId: String) -> OffloadResult {
        ...
        let linuxPath = "\(Self.zeOffloadsLinuxDir)/\(fileName)"
