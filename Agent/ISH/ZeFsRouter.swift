...
//  ZeApp
//
//  Routes guest paths under /var/ze/{offloads,attachments,workspace,browser}
...
//  hook looks up the token here and rewrites to ~/Library/.../ze/<sid>/<bucket>/...
//

import Foundation

final class ZeFsRouter: @unchecked Sendable {
    static let shared = ZeFsRouter()

    /// Guest path prefixes that route per-session. Anything under one of these
    /// becomes ~/Library/.../ze/<sid>/<bucket>/<tail>. Paths under
    /// /var/ze/{memory,skills,shared} stay global and are NOT listed here —
    /// they fall through to the legacy g_bind_mounts[] table.
    private let perSessionBuckets: [(linuxPrefix: String, hostSubdir: String)] = [
        (AIChatViewModel.zeOffloadsLinuxDir,    "offloads"),
        (AIChatViewModel.zeAttachmentsLinuxDir, "attachments"),
        (AIChatViewModel.zeWorkspaceLinuxDir,   "workspace"),
        (AIChatViewModel.zeBrowserLinuxDir,     "browser"),
    ]
    ...
    /// Host base URL (~/Library/ZeChat/ze). Captured once at install time.
    private let zeBaseURL: URL

    private init() {
        self.zeBaseURL = AIChatViewModel.zePersistentBase
    }
    ...
    /// Reverse hook: given a host APFS path under <zeBaseURL>/<sid>/<bucket>,
    /// return the canonical guest path /var/ze/<bucket>/<tail>.
