...
private let logger = AppLogger(category: "AskZeIntent")

/// Siri-facing "ask Ze" entry point. Unlike `SendPromptIntent` (which runs
...
/// conversation — matching the "Hey Siri, ask Ze to …" experience.
///
...
struct AskZeIntent: AppIntent {
    static var title: LocalizedStringResource = "Ask Ze"
    static var description = IntentDescription("Opens Ze, sends your prompt, and shows the conversation. Starts a new session, or continues an existing one when you pick a session.")
    ...
    @Parameter(title: "Prompt", requestValueDialog: "What would you like to ask Ze?")
    var prompt: String
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        if let session = session {
            ...
            if let placeholder = placeholderSid, let realSid = vm.sessionId {
                BackgroundKeepAliveManager.shared.armEagerlyForShortcut(
                    sessionId: realSid, caller: "AskZeIntent.swap")
                SessionActivityTracker.shared.setInactive(placeholder,
                    source: "AskZeIntent.eager.placeholderSwap")
            }
        }
        ...
        logger.info("AskZe send sid=\(sid.prefix(8)) new=\(session == nil)")
        ...
        return .result(dialog: "On it — opening Ze.")
    }
}
