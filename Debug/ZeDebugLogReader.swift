...
/// In-process log reader for the `ze-debug logs` CLI subcommand.
///
/// Unlike the rest of the ze-debug CLI (which routes JSON-RPC through
...
///   2. `LoggingManager`'s daily file (`ze-YYYY-MM-DD.log`) — the same NSLog
...
/// (T-ios-ze-debug-logs-oslogstore).
@objc public final class ZeDebugLogReader: NSObject {

    @objc(sharedInstance)
    public static let shared = ZeDebugLogReader()
