...
enum WebAppPathClassifier {
    ...
    static func classify(hostURL: URL) -> Classified? {
        ...
        //    Library/ZeChat/ze/<sid>/{attachments,workspace}/...
        let zeBase = AIChatViewModel.zePersistentBase.standardizedFileURL.path
        if target.hasPrefix(zeBase + "/") {
            let rel = String(target.dropFirst(zeBase.count + 1))
            // <sid>/<subdir>/<rest>
            ...
            if parts.count >= 3 {
                ...
                default:
                    ...
                    classifierLogger.info("classify: ze subdir not supported sub=\(sub) rel=\(rel)")
                    return nil
                }
            }
            return nil
        ...
        let sharedBase = AIChatViewModel.zeSharedPersistentDir.standardizedFileURL.path
