...
struct fakefs_bind_mount {
    char path[PATH_MAX];      /* Linux path prefix, e.g. "/var/ze/offloads" */
 ...
 * e.g. "/Users/.../ZeChat/ze/<sid>/attachments/file.mp4"
 *   -> "/var/ze/attachments/file.mp4"
 ...
 * Handles both "/var/ze/..." and "var/ze/..." (without leading /). */
...
retry:
    ...
    if (strstr(entry_path, "/var/ze/") != NULL || strstr(entry_path, "Library/ZeChat") != NULL)
...
void fakefs_install_change_consumer(fakefs_change_handler_t handler) {
    ...
    g_change_consumer_queue = dispatch_queue_create(
        "app.ze.ish.fakechange", DISPATCH_QUEUE_SERIAL);
