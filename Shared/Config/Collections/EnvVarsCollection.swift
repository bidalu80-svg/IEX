...
/// `ze://settings/environments?create_key=…` deep link if it wants
...
struct EnvVarsCollection: ConfigCollection {
    ...
    func fields(for id: String) -> [ConfigField] {
        ...
        return [
            ...
            HiddenField(
                ...
                reason: "Env var values are stored in Keychain and never exposed to ze-config"
            ),
        ]
    }
    ...
    func add(_ payload: ConfigValue) throws -> String {
        throw ConfigError.permissionDenied(
            reason: "Use the deep link `ze://settings/environments?create_key=…` so the user enters the value directly"
        )
    }
