...
static NSString *const HELP_TEXT =
    @"apple-alarm - Set alarms and timers on iOS (AlarmKit, iOS 26+)\n"
     ...
     "\n"
     "DEEP LINK:\n"
     "  ze://views/alarm              Open the alarm management page in Ze\n";
...
static dispatch_queue_t authQueue(void) {
    ...
    dispatch_once(&onceToken, ^{
        q = dispatch_queue_create("com.ze.alarm.auth", DISPATCH_QUEUE_SERIAL);
    });
    return q;
}
    ...
    if (!authorized) {
        NSString *msg = authError
            ? [NSString stringWithFormat:@"AlarmKit authorization failed: %@. "
                "To grant access, open Settings > Privacy & Security > Alarms and enable Ze.",
...
static int cmd_set_alarmkit(int argc, char **argv, int stdout_fd,
    ...
    if (@available(iOS 26.0, *)) {
        ...
        data[@"view_url"] = @"ze://views/alarm";
        data[@"hint"] = @"Alarm is now visible on the Ze home screen. Open ze://views/alarm to manage alarms.";
...
static int cmd_timer_alarmkit(int argc, char **argv, int stdout_fd,
    ...
    if (@available(iOS 26.0, *)) {
        ...
        data[@"view_url"] = @"ze://views/alarm";
        data[@"hint"] = @"Timer is now visible on the Ze home screen. Open ze://views/alarm to manage alarms.";
