#!/usr/bin/env python3
"""ze-mcp-cli — MCP client CLI for the Ze agent (iSH / PRoot).
...
Diagnostics go to /var/ze/mcp-servers/mcp-cli.log, never to stdout.
...
published in /tmp/ze-mcp-daemon.port. info / add / remove / enable / disable
...
LOG_PATH = "/var/ze/mcp-servers/mcp-cli.log"

PID_FILE = "/tmp/ze-mcp-daemon.pid"
PORT_FILE = "/tmp/ze-mcp-daemon.port"  # daemon publishes its 127.0.0.1 port here
LOCK_FILE = "/tmp/ze-mcp-daemon.lock"  # cold-start fork guard (one winner forks)
...
def maybe_start_daemon():
    ...
    try:
        ...
        import logging
        daemon_log = "/var/ze/mcp-servers/mcp-daemon.log"
        try:
...
def cmd_add(args, pretty):
    ...
    if wants_oauth:
        out["oauth_note"] = (
            ...
            "the CLI — open Ze Settings > MCP Integrations > %s and tap "
...
USAGE = """ze-mcp-cli — MCP (Model Context Protocol) client for the Ze agent.

Usage: ze-mcp-cli <command> [args] [--pretty]
...
Files:
  Servers:  /var/ze/mcp-servers/servers.json   (mcpServers object, Claude-Desktop compatible)
  Log:      /var/ze/mcp-servers/mcp-cli.log

Examples:
  ze-mcp-cli list --pretty
  ze-mcp-cli tools notion
  ze-mcp-cli call notion search --input '{"q":"x"}'
  ze-mcp-cli add --name notion --url https://mcp.notion.so/mcp --header "Authorization: Bearer $NOTION_TOKEN"
  ze-mcp-cli add --name github --command npx --args "-y @modelcontextprotocol/server-github" --env "GITHUB_TOKEN=$GITHUB_TOKEN"
  ze-mcp-cli add --name atlassian --command uvx --args "mcp-atlassian" --startup-timeout 120
  ze-mcp-cli add --name gworkspace --url https://my-gws-mcp.example.com/mcp \
...
Server startup timeout (Ze config, not part of the MCP protocol):
...
def main():
    ...
    try:
        if cmd == "list":
            ...
            sys.stderr.write("Run 'ze-mcp-cli --help' for usage.\n")
