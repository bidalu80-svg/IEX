...
/// Used by both the agent loop (AIChatViewModel) and ze-model-use offload bridge.
@MainActor
enum LLMProviderFactory {
    ...
    static func applyCustomUserAgent(_ provider: OpenAIProvider, instance: ProviderInstance) -> OpenAIProvider {
        ...
        // marketing version (Ze/1.10 …) instead of URLSession's build-number
        // default (Ze/1 CFNetwork/… Darwin/…). Don't clobber a UA another
        // builder already set (e.g. some future provider-specific UA).
        if provider.extraHeaders["User-Agent"] == nil {
            provider.extraHeaders["User-Agent"] = ZeUserAgent.default
        }
        return provider
    }
    ...
    static func makeAnthropicProvider(instance: ProviderInstance, model: LLMModel) -> AnthropicProvider {
        ...
        case .apiKey:
            ...
            // No user custom UA → send the app default (Ze/<marketing>) so the
            ...
            return AnthropicProvider(apiKey: key, model: model, basePath: customBase, appendV1Suffix: appendV1, customUserAgent: ua ?? ZeUserAgent.default)
        case .oauth:
