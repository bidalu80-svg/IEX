...
private let zeLogger = AppLogger(category: "ZeMedia")
// MARK: - ze:// Media Provider

// MARK: Media Cache — avoids re-loading/decoding when LazyVStack recycles cells

final class ZeMediaCache {
    static let shared = ZeMediaCache()

    /// Downsampled UIImage cache keyed by absolute ze:// URL string.
    private let imageCache = NSCache<NSString, UIImage>()
    /// Video thumbnail cache keyed by file path.
    private let thumbnailCache = NSCache<NSString, UIImage>()
    /// Resolved host file URL cache keyed by ze:// URL string.
...
/// Resolve a ze:// URL with caching to avoid repeated FileManager lookups.
///
/// Cache key is scoped by the active session id because per-session paths
/// (e.g. `ze://attachments/foo.png`) resolve to different files in
/// `Library/ZeChat/ze/<sessionId>/…` depending on which session is
...
func resolveZeFileURLCached(url: URL) -> URL? {
    let sessionScope = AIChatViewModel.activeSessionId ?? "nil"
    let key = "\(sessionScope)|\(url.absoluteString)"
    if let cached = ZeMediaCache.shared.resolvedURL(for: key) {
        zeLogger.info("[ZeImage][ResolveCache] HIT url=\(url.absoluteString) → \(cached.path)")
        return cached
    }
    if let resolved = resolveZeFileURL(url: url) {
        ZeMediaCache.shared.setResolvedURL(resolved, for: key)
        zeLogger.info("[ZeImage][ResolveCache] MISS→resolved url=\(url.absoluteString) → \(resolved.path)")
        return resolved
    }
    zeLogger.warning("[ZeImage][ResolveCache] MISS→nil url=\(url.absoluteString)")
    return nil
}
...
///
/// For `ze://` sources we resolve to the host file URL and stat it.
...
func zeMediaCacheKey(for source: String) -> String {
    guard let url = URL(string: source) else { return source }
    return zeMediaCacheKey(for: url)
}

func zeMediaCacheKey(for url: URL) -> String {
    let base = url.absoluteString
    // Non-ze schemes: return as-is, we don't own that file's lifecycle.
    guard url.scheme == "ze" else { return base }
    guard let fileURL = resolveZeFileURLCached(url: url) else {
...
let zeMarkdownExtensions: Set<String> = ["md", "markdown"]
let zeHTMLExtensions: Set<String> = ["html", "htm", "svg"]
let zeDocumentExtensions: Set<String> = [
    "pdf",
    // Microsoft Office
...
/// Resolve a ze:// URL to a host filesystem URL.
/// Maps directly to persistent storage: Library/ZeChat/ze/<sessionId>/<subdir>/<path>
/// No dependency on iSH boot or bind mounts.
func resolveZeFileURL(url: URL) -> URL? {
    guard let host = url.host else {
        zeLogger.warning("[ResolveZeURL] no host in URL: \(url.absoluteString)")
        return nil
    }
    ...
    if let sid = AIChatViewModel.activeSessionId {
        for subPath in subPaths {
            ...
            zeLogger.info("[ZeImage][Resolve] \(url.absoluteString) → session=\(sid) → \(persistURL.path) exists=\(exists) size=\(fileSize)")
    ...
    let globalDirs: [(subdir: String, url: URL)] = [
        ("skills", AIChatViewModel.zeSkillsPersistentDir),
        ("memory", AIChatViewModel.zeMemoryPersistentDir),
        ("shared", AIChatViewModel.zeSharedPersistentDir),
    ]
    for (subdir, globalDir) in globalDirs where host == subdir {
        for subPath in subPaths {
            let candidate = globalDir.appendingPathComponent(subPath)
            if fm.fileExists(atPath: candidate.path) {
                zeLogger.info("[ZeImage][Resolve] \(url.absoluteString) → global \(subdir) → \(candidate.path)")
                return candidate
            }
        }
    }

    // User-mounted external folders: ze://mounts/<name>/<path>
    ...
    if host == "mounts" {
        for subPath in subPaths {
            ...
            if let mountURL {
                ...
                if fm.fileExists(atPath: candidate.path) {
                    zeLogger.info("[ZeImage][Resolve] \(url.absoluteString) → mount '\(mountName)' → \(candidate.path)")
                    return candidate
                }
            }
        }
    ...
    // [T-ios-zeurl-cross-session-isolation] DO NOT scan other sessions.
    // ze://attachments/<file> is session-scoped: it must only resolve
    ...
    // fallback was a P0 isolation leak — ze-model-use emits fixed names like
    ...
    zeLogger.warning("[ZeImage][Resolve] \(url.absoluteString) → not found in active session, global dirs, or mounts (cross-session scan disabled for isolation)")
    return nil
}

// MARK: - ze:// Link Handling

/// SF Symbol icon for a given file extension.
private func zeFileIcon(for ext: String) -> String {
...
/// Tappable file chip for ze:// links in Markdown.
private struct ZeFileChipView: View {
    let url: URL
    ...
    var body: some View {
        ...
        Button {
            ...
            HStack(spacing: 6) {
                Image(systemName: zeFileIcon(for: ext))
                    .font(.system(size: 13))
...
struct AsyncImageTile: View {
    ...
    var body: some View {
        ...
        .task(id: zeMediaCacheKey(for: meta.zeURL)) { loadThumbnail() }
    }

    private func loadThumbnail() {
        // Fingerprint key captures file size + mtime so a rewrite of the
        // same ze:// path invalidates the cache automatically.
        let cacheKey = "attach:\(zeMediaCacheKey(for: meta.zeURL))"
...
/// Environment action for handling ze:// and http(s):// URL taps.
///
...
private struct ZeURLKey: EnvironmentKey {
    static let defaultValue = ZeURLAction { _ in .systemAction }
}

extension EnvironmentValues {
    var openZeURL: ZeURLAction {
        get { self[ZeURLKey.self] }
        set { self[ZeURLKey.self] = newValue }
    }
}

/// Thin modifier that overrides `\.openURL` for a subtree and forwards to the
/// top-level `\.openZeURL` action. Keeps cell-internal views from holding their
/// own sheet @State (which would break presentation animations).
struct ZeOpenURLHandler: ViewModifier {
    @Environment(\.openZeURL) private var openZeURL
...
private struct ZeImageView: View {
    ...
    var body: some View {
        if let url, url.scheme == "ze" {
            let ext = url.pathExtension.lowercased()
            let _ = zeLogger.info("[ZeImage][View] render url=\(url.absoluteString) ext=\(ext) hasLoadedImage=\(self.loadedImage != nil) attempt=\(self.loadAttempt)")
            if zeAudioExtensions.contains(ext) {
                ...
                zeImageContent(url: url)
                    ...
                    // already fingerprint-keyed via zeMediaCacheKey.
    ...
    @ViewBuilder
    private func zeImageContent(url: URL) -> some View {
        let maxH = UIScreen.main.bounds.height / 2
    ...
    private func loadWithRetry(url: URL) async {
        let cacheKey = zeMediaCacheKey(for: url)
        zeLogger.info("[ZeImage][MarkdownUI] loadWithRetry START url=\(url.absoluteString)")
        // Check memory cache first — instant on cell recycle
        if let cached = ZeMediaCache.shared.image(for: cacheKey) {
            zeLogger.info("[ZeImage][MarkdownUI] CACHE HIT url=\(url.absoluteString) size=\(cached.size.width)x\(cached.size.height)")
            loadedImage = cached
        ...
        for attempt in 0..<6 {
            ...
            loadAttempt = attempt
            if let fileURL = resolveZeFileURLCached(url: url) {
                zeLogger.info("[ZeImage][MarkdownUI] attempt=\(attempt) url=\(url.absoluteString) resolved=\(fileURL.path) exists=\(FileManager.default.fileExists(atPath: fileURL.path))")
                // Load and downsample on background thread to avoid blocking main thread
                let image = await Task.detached(priority: .userInitiated) {
                    guard let data = try? Data(contentsOf: fileURL) else {
                        zeLogger.warning("[ZeImage][MarkdownUI] Data(contentsOf:) FAILED url=\(url.absoluteString) path=\(fileURL.path)")
                        return nil as UIImage?
                    }
                    let fileSize = data.count
                    let img = downsampleImage(data: data, maxPixelSize: 2048)
                    if let img {
                        zeLogger.info("[ZeImage][MarkdownUI] decoded url=\(url.absoluteString) dataSize=\(fileSize) imgSize=\(img.size.width)x\(img.size.height)")
                    } else {
                        zeLogger.warning("[ZeImage][MarkdownUI] downsample FAILED url=\(url.absoluteString) dataSize=\(fileSize)")
                    }
                    return img
                }.value
                if let image {
                    ZeMediaCache.shared.setImage(image, for: cacheKey)
                    loadedImage = image
                    zeLogger.info("[ZeImage][MarkdownUI] SUCCESS url=\(url.absoluteString) attempt=\(attempt)")
                    // [T-attachment-size-invalidate] Placeholder (~fixed) → fit (variable, up to half-screen).
                    notifySizeChangeOnce()
                    return
                }
            } else {
                zeLogger.warning("[ZeImage][MarkdownUI] resolve FAILED attempt=\(attempt) url=\(url.absoluteString)")
            }
        }
        zeLogger.error("[ZeImage][MarkdownUI] GAVE UP after 6 attempts url=\(url.absoluteString)")
        loadAttempt = 0
    }

    /// Post .zeAttachmentSizeChanged at most once per view lifetime.
    ...
    private func notifySizeChangeOnce() {
        ...
        NotificationCenter.default.post(name: .zeAttachmentSizeChanged, object: urlStr)
    }
}
...
// MARK: - ze:// Image File Preview (from URL handler)

/// Fullscreen image preview loading from a local file URL.
struct ZeImageFilePreviewView: View {
    let fileURL: URL
...
/// `.task(id: zeMediaCacheKey(...))`; mirror it here with a cheap stat so a
/// same-path rewrite re-reads the file. Unlike zeMediaCacheKey this takes the
/// already-resolved disk URL directly (no ze:// re-resolve).
...
private struct ZeTextView: UIViewRepresentable {
    let text: String
...
// Non-private so cross-file preview sheets (e.g. ZeLinkPreviewView in
...
struct ZeMarkdownPreviewView: View {
    let fileURL: URL
    @Environment(\.dismiss) private var dismiss
...
// MARK: - ze:// HTML Preview

struct ZeHTMLPreviewView: View {
    let fileURL: URL
    ...
    var body: some View {
        ...
        // iPad/Mac, matching ZeMarkdownPreviewView. iPhone unaffected.
        .modifier(WideSheetSizingModifier())
...
// MARK: - ze:// Document Preview

import QuickLook

struct ZeDocumentPreviewView: UIViewControllerRepresentable {
    let fileURL: URL
    ...
    class Coordinator: NSObject, QLPreviewControllerDataSource {
        ...
        /// zeMediaCacheKey's stat-based approach.
