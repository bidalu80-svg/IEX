...
struct WebLoadError: Equatable {
    ...
    init?(error: Error, failedURL: URL? = nil) {
        ...
        case (NSURLErrorDomain, NSURLErrorCannotFindHost),
            ...
            message = host.map {
                String(localized: "Ze can’t open the page because it can’t find the server “\($0)”.")
            } ?? String(localized: "Ze can’t open the page because it can’t find the server.")
            systemImage = "wifi.exclamationmark"

        case (NSURLErrorDomain, NSURLErrorCannotConnectToHost):
            title = String(localized: "Cannot Open Page")
            message = String(localized: "Ze can’t open the page because it can’t connect to the server.")
            systemImage = "wifi.exclamationmark"
        ...
        case (NSURLErrorDomain, NSURLErrorSecureConnectionFailed),
            ...
            message = host.map {
                String(localized: "Ze can’t verify the identity of the server “\($0)”.")
            } ?? String(localized: "Ze can’t verify the identity of the server.")
