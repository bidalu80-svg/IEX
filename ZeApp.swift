...
struct ZeApp: App {
    ...
    init() {
        ...
        // first render instead of the `.default` stub ("Ze"). Without this the
        ...
        AgentLiveActivityManager.shared.cleanupStaleActivities(source: "ZeApp.init")
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                ...
                .id(appLanguage)
                // Confirmation gate for every ze-config write. Mounted
                ...
                // for `ze://open?session=…&path=…`). Mounted at the
                ...
                .onAppear {
                    ...
                    if #available(iOS 17.0, *) {
                        ZeShortcutsProvider.updateAppShortcutParameters()
                    }
                    ...
                    // Create /var/ze/mounts/<name> symlinks in the fakefs now
    ...
    private static let fileProviderDomain = NSFileProviderDomain(
        identifier: NSFileProviderDomainIdentifier("com.ze.app.files"),
        displayName: "Ze"
    )
    ...
    private static func registerFileProviderDomain() {
        ...
        NSFileProviderManager.getDomainsWithCompletionHandler { domains, error in
            ...
            if needsForceReset && alreadyRegistered {
                ...
                // App Group `ZeFileProvider/` path that we hardcoded
                // and that iOS does not own. So `.removeAll` SHOULD leave
                // our data intact, but to be safe we snapshot to
                // `ZeConfig/_reset_backup_v3/` first and restore if we
                ...
                let backupRoot = AIChatViewModel.zeConfigRoot.appendingPathComponent("_reset_backup_v3", isDirectory: true)
                ...
                NSFileProviderManager.remove(fileProviderDomain, mode: .removeAll) { preservedLocation, removeErr in
                    ...
                    let providerRoot = AIChatViewModel.zeAppGroupRoot
                    let fm = FileManager.default
            ...
            let stale = domains.filter { $0.identifier.rawValue.contains("com.ze") }
    ...
    /// 1. Library/ZeChat/shared/ → App Group ZeFileProvider/shared/
    /// 2. Library/ZeChat/memory/ → App Group ZeFileProvider/memory/
    /// 3. Library/ZeChat/skills/ → App Group ZeFileProvider/skills/
    /// 4. Old App Group ZeShared/ → App Group ZeFileProvider/shared/
    private static func migrateSharedDirToAppGroup() {
        let fm = FileManager.default
        let library = fm.urls(for: .libraryDirectory, in: .userDomainMask).first!

        var migrations: [(source: URL, dest: URL, label: String)] = [
            // Legacy Library/ZeChat/shared → new shared
            (library.appendingPathComponent("ZeChat/shared", isDirectory: true),
             AIChatViewModel.zeSharedPersistentDir, "shared"),
            // Legacy Library/ZeChat/memory → new memory
            (library.appendingPathComponent("ZeChat/memory", isDirectory: true),
             AIChatViewModel.zeMemoryPersistentDir, "memory"),
            // Legacy Library/ZeChat/skills → new skills
            (library.appendingPathComponent("ZeChat/skills", isDirectory: true),
             AIChatViewModel.zeSkillsPersistentDir, "skills"),
        ]

        if let container = SharedContainerStore.appGroupContainerURL {
            migrations.append((
                container.appendingPathComponent("ZeShared", isDirectory: true),
                AIChatViewModel.zeSharedPersistentDir,
                "ZeShared→shared"
            ))
        }
    ...
    /// `ze://open?session=…&path=…` deep link (ze.app launcher
